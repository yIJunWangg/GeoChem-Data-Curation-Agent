...
def test_schema_table_order_contains_fts_after_documents():
    ...
    assert tables.index("retrieval_chunk_fts") == tables.index("retrieval_chunks") + 1
...
def test_sqlite_tables_omits_internal_fts_shadow_tables():
    ...
    try:
        ...
        connection.execute("CREATE TABLE retrieval_chunks (chunk_id TEXT)")
        connection.execute("CREATE VIRTUAL TABLE retrieval_chunk_fts USING fts5(chunk_id, content)")

        tables = sqlite_tables(connection)

        assert "projects" in tables
        assert "retrieval_fts" in tables
        assert "retrieval_chunk_fts" in tables
